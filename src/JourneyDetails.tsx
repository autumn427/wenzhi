...
function SkillRadar({run, baseline = 'previous'}:{run:UniverseRun; baseline?: 'previous' | 'initial' | 'timeline'}) {
 ...
 const axes:ReadonlyArray<readonly [keyof SimulationState,string]> = campus ? [['technicalSkill','执行'],['aiCollaboration','协作'],['domainDepth','判断'],['portfolio','成果'],['opportunity','机会'],['energy','精力']] : [['technicalSkill','技术'],['aiCollaboration','AI 协作'],['domainDepth','专业'],['portfolio','作品'],['opportunity','机会'],['energy','精力']]
 ...
 useEffect(()=>{
  ...
  const paperTag=(text:string,x:number,y:number,font:string,color:string,kind:'axis'|'value'='value')=>{ctx.save();ctx.font=font;if(kind==='axis'){ctx.fillStyle=color;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(text,x,y);ctx.restore();return}const width=ctx.measureText(text).width+14;const height=21;ctx.translate(x,y);ctx.shadowColor='rgba(93,69,37,.20)';ctx.shadowBlur=4;ctx.shadowOffsetX=2;ctx.shadowOffsetY=3;ctx.fillStyle='#fffaf0';ctx.beginPath();ctx.moveTo(-width/2,-height/2+2);ctx.lineTo(-width/2+4,-height/2);ctx.lineTo(width/2-3,-height/2+1);ctx.lineTo(width/2,-height/2+4);ctx.lineTo(width/2-1,height/2-2);ctx.lineTo(width/2-4,height/2);ctx.lineTo(-width/2+2,height/2-1);ctx.closePath();ctx.fill();ctx.shadowColor='transparent';ctx.strokeStyle='#eadfce';ctx.lineWidth=1;ctx.stroke();ctx.fillStyle=color;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(text,0,0);ctx.restore()}
...
export function JourneyTickets({run,startIndex=0}:{run:UniverseRun;startIndex?:number}) {
 const labels = !run.route && run.currentEvent.id.startsWith('campus-') ? {...ticketLabels,technicalSkill:'执行',aiCollaboration:'协作',domainDepth:'判断',portfolio:'成果'} : ticketLabels
 return <section className="journey-tickets" aria-label="每次选择的故事票根">
  {run.decisions.length ? run.decisions.map((decision,index)=> {
   ...
   return <article className="journey-ticket" key={`${run.code}-${decision.eventId}`}>
    ...
    <div className="ticket-changes" aria-label="这次选择的能力变化">{changes.length ? changes.map(([key,value])=><span key={key} className={value>0?'up':'down'}>{labels[key]??key} {value>0?'+':''}{value}</span>) : <span>能力值未变化</span>}</div>
   </article>
